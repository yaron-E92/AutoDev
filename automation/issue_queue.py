...
def _run_gh(
    ...
    *,
    ...
    if check and result.returncode != 0:
        ...
        raise QueueError(f"GitHub command failed ({result.returncode}): {' '.join(result.argv)}: {detail}")
    return result
...
def list_blockers(
    ...
    *,
    ...
    endpoint = f"repos/{github_repo}/issues/{issue_number}/dependencies/blocked_by?per_page=100"
    result = _run_gh(
        repo,
...
def remove_dependency(
    ...
    *,
    ...
    endpoint = f"repos/{github_repo}/issues/{issue_number}/dependencies/blocked_by/{blocker_id}"
    _run_gh(
        repo,
        [
            "api",
...
def ensure_queue_labels(
    ...
    *,
    ...
    result = _run_gh(
        repo,
        ["label", "list", "--repo", github_repo, "--limit", "1000", "--json", "name"],
        runner=runner,
    )
...
def _split_blockers(blockers: list[Blocker]) -> tuple[tuple[Blocker, ...], tuple[Blocker, ...]]:
...
def inspect_queue(
    ...
    *,
    ...
    for issue in list_issues(repo, github_repo, limit=limit, runner=runner):
        labels = set(issue.labels)
        if not labels.intersection({MANAGED_LABEL, READY_LABEL, BLOCKED_LABEL, ATTENTION_LABEL, RUNNING_LABEL}):
            continue
        blockers: list[Blocker] = []
        ...
        if issue.state == "open" and MANAGED_LABEL in labels:
            ...
            for blocker in blockers:
                if blocker.state == "open":
                    continue
                remove_dependency(repo, github_repo, issue.number, blocker.id, runner=runner)
...
def queue_summary(states: list[QueueState]) -> dict[str, int]:
    ...
    return {
        ...
        "dependency_blocked": sum(state.reason == "blocked" for state in open_managed),
        "attention_required": sum(ATTENTION_LABEL in state.issue.labels for state in open_managed),
        "running": sum(state.reason == "running" for state in open_managed),
        "policy_excluded": sum(state.reason == "policy-excluded" for state in open_managed),
    }
...
def explain_state(state: QueueState) -> str:
    ...
    if state.reason == "blocked":
        blockers = ", ".join(f"#{item.number} {item.title}".strip() for item in state.open_blockers)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="autodev queue",
        description="Maintain AutoDev issue queue state without model calls.",
    )
    ...
    *,
    ...
    try:
        ...
        if args.command == "reconcile":
            states, created = reconcile_queue(current, github_repo, limit=args.limit, runner=runner)
            if args.json:
                print(
                    json.dumps(
                        {
                            ...
                            "changed_issues": [state.issue.number for state in states if state.changed],
                            "closed_dependencies_removed": {
                                str(state.issue.number): list(state.removed_closed_dependencies)
                                for state in states
                ...
                removed = sum(len(state.removed_closed_dependencies) for state in states)
                print(
                    f"AutoDev queue reconciled for {github_repo}: "
                    f"managed={summary['managed']} ready={summary['ready']} "
                    f"blocked={summary['dependency_blocked']} attention={summary['attention_required']} "
                    f"changed={changed} closed-dependencies-removed={removed}",
                    file=out,
                )
            return 0
        if args.command == "status":
            states = inspect_queue(current, github_repo, limit=args.limit, runner=runner)
            summary = queue_summary(states)
            if args.json:
                print(json.dumps({"repository": github_repo, "summary": summary}, sort_keys=True), file=out)
            else:
                print(
                    f"AutoDev queue status for {github_repo}: "
                    f"managed={summary['managed']} ready={summary['ready']} "
                    f"blocked={summary['dependency_blocked']} attention={summary['attention_required']} "
                    f"running={summary['running']} policy-excluded={summary['policy_excluded']}",
                    file=out,
                )
            return 0
