...
class DocumentationContractTests(unittest.TestCase):
    ...
    def test_release_guide_matches_native_and_source_artifact_set(self) -> None:
        ...
        self.assertIn("self-signed AutoDev code-signing certificate", releases)
        self.assertIn("AUTODEV_WINDOWS_SIGNING_PFX_BASE64", releases)
        self.assertIn("AUTODEV_WINDOWS_SIGNING_PFX_PASSWORD", releases)
        self.assertIn("final signed MSI bytes", releases)
        self.assertIn("#197", releases)
        self.assertNotIn("signing-ready but unsigned", releases)
