...
Run the installed bridge as `python .opencode/autodev.py resume` (or `python3` where required). If `$ARGUMENTS` contains one or more `--invalidate-role <role>` values, append only those explicit invalidations. Do not pass model override flags; model changes come from the normal OpenCode configuration resolved by #66.
