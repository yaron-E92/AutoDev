...
**Native structured-output override:** when the Python-coordinator prompt additionally says the invocation is bound to an AutoDev structured-output contract and the runtime requests JSON-schema output, apply the targeted repair exactly as normal and return only the bounded completion report through the runtime's structured-output mechanism: `completion_summary` plus `validation_notes`. The report is non-authoritative evidence. Do not claim that the repair succeeded, a test passed, source identity changed, or a verification/workflow stage completed merely because the report is schema-valid. AutoDev Python computes source identity and reruns the appropriate deterministic or semantic verification stage independently.
