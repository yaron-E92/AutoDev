...
- `BLOCKED`: run `stage --name blocked --reason "<reason>"` through the bridge, then finish `BLOCKED`.
- `FAILED`: run `stage --name failed --reason "<reason>"` through the bridge, then finish `FAILED`.
...
6. Deterministic verification
   ...
   - On `BLOCKED`, run `stage --name blocked --reason "<reason>"`, then finish `BLOCKED`.
   - On Task/bridge failure, run `stage --name failed --reason "<reason>"`, then finish `FAILED`.
